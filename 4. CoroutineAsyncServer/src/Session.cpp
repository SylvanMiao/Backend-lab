...
Session::Session(boost::asio::io_context &io_context, Server *server) : _socket(io_context), _server(server), _b_close(false)
{
...
void Session::Start()
{
	...
	// 开启接收协程
	boost::asio::co_spawn(_socket.get_executor(), [shared_this, this]() -> boost::asio::awaitable<void>
												{
		try {
			for (;!_b_close;) {
				_recv_head_node->Clear();
			    ...
			    n = co_await boost::asio::async_read(_socket,
					boost::asio::buffer(_recv_msg_node->_data, _recv_msg_node->_total_len), boost::asio::use_awaitable);
